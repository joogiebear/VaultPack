...
public class MMOStatReward extends NumberReward {
    @Getter
    private static final String MMO_STAT = "aurora_collections";
    private String stat;
    ...
    public void execute(Player player, long level, List<Placeholder<?>> placeholders) {
        ...
        Bukkit.getGlobalRegionScheduler().runDelayed(Bukkit.getPluginManager().getPlugin("AuroraCollections"), (task) -> {
    ...
    public void init(ConfigurationSection args) {
        ...
        if (this.stat == null) {
            this.valid = false;
            AuroraCollections.logger().warning("Stat is not defined in MMOStatReward");
        }
